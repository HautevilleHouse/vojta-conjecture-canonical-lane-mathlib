/-!
# Source package model for `vojta-conjecture-canonical-lane`
...
def sourceCheckoutHead : String := "d599c729996263e99d6f7549216b3e1a2b3d4232"
...
def sourceFiles : List SourceFile := [
  ...
  { path := "scripts/voj_closure_guard.py", sha256 := "dfc3d30b7de117acbffcc07124e94e5e8ba1d9954ce8d538893195dd87621e3a", functionCount := 9, classCount := 0, assignmentCount := 8, parseOk := true }
]
...
def sourceFunctions : List SourceFunctionDecl := [
  ...
  { file := "scripts/voj_closure_guard.py", name := "_finite", args := ["v"], returns := "bool", doc := "", line := 31, isAsync := false },
  { file := "scripts/voj_closure_guard.py", name := "_resolve", args := ["path_str"], returns := "Path", doc := "", line := 35, isAsync := false },
  { file := "scripts/voj_closure_guard.py", name := "_bootstrap_registry", args := ["path"], returns := "None", doc := "", line := 42, isAsync := false },
  { file := "scripts/voj_closure_guard.py", name := "_load_registry", args := ["path"], returns := "dict[str, Any]", doc := "", line := 48, isAsync := false },
  { file := "scripts/voj_closure_guard.py", name := "_load_sigma_from_stitch", args := ["path"], returns := "float | None", doc := "", line := 60, isAsync := false },
  { file := "scripts/voj_closure_guard.py", name := "_entry", args := ["constants", "key"], returns := "tuple[float | None, bool]", doc := "", line := 77, isAsync := false },
  { file := "scripts/voj_closure_guard.py", name := "compute_report", args := ["data", "sigma_star", "strict_coh_zero"], returns := "dict[str, Any]", doc := "", line := 88, isAsync := false },
  { file := "scripts/voj_closure_guard.py", name := "append_history", args := ["path", "report"], returns := "None", doc := "", line := 121, isAsync := false },
  { file := "scripts/voj_closure_guard.py", name := "main", args := [], returns := "None", doc := "", line := 128, isAsync := false }
]
...
def sourceAssignments : List SourceAssignmentDecl := [
  ...
  { file := "scripts/voj_closure_guard.py", name := "SCRIPT_DIR", value := "Path(__file__).resolve().parent", line := 13 },
  { file := "scripts/voj_closure_guard.py", name := "PROJECT_ROOT", value := "SCRIPT_DIR.parent", line := 14 },
  { file := "scripts/voj_closure_guard.py", name := "DEFAULT_REGISTRY", value := "\"artifacts/constants_registry.json\"", line := 16 },
  { file := "scripts/voj_closure_guard.py", name := "DEFAULT_STITCH", value := "\"artifacts/stitch_constants.json\"", line := 17 },
  { file := "scripts/voj_closure_guard.py", name := "DEFAULT_OUT", value := "\"repro/certificate_runtime.json\"", line := 18 },
  { file := "scripts/voj_closure_guard.py", name := "DEFAULT_HISTORY", value := "\"repro/drift_guard_runs.jsonl\"", line := 19 },
  { file := "scripts/voj_closure_guard.py", name := "PRIMARY_KEYS", value := "[\"kappa_height\", \"sigma_counting\", \"kappa_compact\", \"rho_rigidity\", \"inequality_transfer\"]", line := 21 },
  { file := "scripts/voj_closure_guard.py", name := "COHERENCE_KEY", value := "\"eps_coh\"", line := 28 }
]

def constantSpecs : List ConstantSpec := [
  { group := "constants", key := "kappa_height", status := "derived_numeric", formula := "c_height_raw * height_density_raw - e_height_raw", sourceSection := "paper/VOJTA_CONJECTURE_PREPRINT.md Section 4.5 / Appendix A", validation := "required_positive", componentKeys := ["c_height_raw", "e_height_raw", "height_density_raw"] },
  { group := "constants", key := "sigma_counting", status := "derived_numeric", formula := "counting_floor_raw - truncation_loss_raw - restart_loss_raw", sourceSection := "paper/VOJTA_CONJECTURE_PREPRINT.md Section 5.2 / Appendix B", validation := "required_positive", componentKeys := ["counting_floor_raw", "restart_loss_raw", "truncation_loss_raw"] },
  { group := "constants", key := "kappa_compact", status := "derived_numeric", formula := "1.0 / (1.0 + delta_comp_sup_raw)", sourceSection := "paper/VOJTA_CONJECTURE_PREPRINT.md Section 5.3 / Appendix C", validation := "required_positive", componentKeys := ["delta_comp_sup_raw"] },
  { group := "constants", key := "rho_rigidity", status := "derived_numeric", formula := "rho_rigidity_raw", sourceSection := "paper/VOJTA_CONJECTURE_PREPRINT.md Section 6.1 / Appendix D", validation := "required_positive", componentKeys := ["rho_rigidity_raw"] },
  { group := "constants", key := "inequality_transfer", status := "derived_numeric", formula := "c_ineq_raw * transfer_gain_raw - e_ineq_raw", sourceSection := "paper/VOJTA_CONJECTURE_PREPRINT.md Section 6.2 / Appendix E.4", validation := "required_positive", componentKeys := ["c_ineq_raw", "e_ineq_raw", "transfer_gain_raw"] },
  { group := "constants", key := "eps_coh", status := "derived_numeric", formula := "eps_coh_raw", sourceSection := "paper/VOJTA_CONJECTURE_PREPRINT.md Appendix E.6", validation := "required_nonnegative", componentKeys := ["eps_coh_raw"] },
  { group := "stitch", key := "sigma_star_can", status := "derived_numeric", formula := "sigma_star_can_raw", sourceSection := "paper/VOJTA_CONJECTURE_PREPRINT.md Section 5 / stitch constants", validation := "required_positive", componentKeys := ["sigma_star_can_raw"] }
]

def registryConstants : List RegistryConstant := [
  { key := "eps_coh", value := "0.0", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#eps_coh", sourceSection := "paper/VOJTA_CONJECTURE_PREPRINT.md Appendix E.6" },
  { key := "inequality_transfer", value := "1.029451", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#inequality_transfer", sourceSection := "paper/VOJTA_CONJECTURE_PREPRINT.md Section 6.2 / Appendix E.4" },
  { key := "kappa_compact", value := "0.8045052292839904", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#kappa_compact", sourceSection := "paper/VOJTA_CONJECTURE_PREPRINT.md Section 5.3 / Appendix C" },
  { key := "kappa_height", value := "1.092899", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#kappa_height", sourceSection := "paper/VOJTA_CONJECTURE_PREPRINT.md Section 4.5 / Appendix A" },
  { key := "rho_rigidity", value := "1.077", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#rho_rigidity", sourceSection := "paper/VOJTA_CONJECTURE_PREPRINT.md Section 6.1 / Appendix D" },
  { key := "sigma_counting", value := "1.073", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#sigma_counting", sourceSection := "paper/VOJTA_CONJECTURE_PREPRINT.md Section 5.2 / Appendix B" }
]
